...
- **Batch Processing**: Text chunks are embedded in batches of 100 (`batch_size=100`) via Ollama to optimize throughput for both single files and directories.
...
- **Database Naming**: Names must match `[A-Za-z0-9_-]+` (letters, numbers, underscores, and hyphens) and cannot be reserved system names.
...
>
> **TUI Status Indicator**: The REPL header displays `RAG: <name>` when a database is loaded.
...
```python
# Defined in ollama_agent/agent/builtin_tools.py
@tool
...
4. **Context Injection**: Relevant text chunks and source file headers (`[Source: filename.py]`) are returned in `RAGToolResult.context` separated by `---` dividers.
